...
public class PacketOutUserInfoTest extends PacketServerTest {
    ...
    @Test
    public void globalSelfPackagingTest() {
        Mockito.when(this.user.getUserId()).thenReturn(randomUniqueId());
        Mockito.when(this.user.getUsername()).thenReturn(randomString());
        Mockito.when(this.user.getStatus()).thenReturn(randomEnum(Status.class));
        Mockito.when(this.user.getWorld()).thenReturn(null);

        final PacketOutUserInfo packet = this.getPacket(PacketOutUserInfo.class, this.user);

        Assert.assertNull(packet.getContextId());
        Assert.assertEquals(Action.UPDATE_USER, packet.getAction());
        Assert.assertEquals(this.user.getUserId(), packet.getInfo().getUserId());
        Assert.assertEquals(this.user.getUsername(), packet.getInfo().getName());
        Assert.assertEquals(this.user.getStatus(), packet.getInfo().getStatus());
        Assert.assertTrue(packet.getInfo().getFlags().isEmpty());
    }
    ...
    @Test
    public void worldRemoveFriendPackagingTest() {
        final IUser target = Mockito.mock(IUser.class);
        final IWorld world = Mockito.mock(IWorld.class);
        final Map<UUID, IUser> friends = new HashMap<>();
