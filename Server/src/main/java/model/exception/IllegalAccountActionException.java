package model.exception;

public class IllegalAccountActionException extends Exception {
    private final String clientMessageKey;

    public IllegalAccountActionException(String errorMessage, String clientMessageKey) {
        super(errorMessage);
        this.clientMessageKey = clientMessageKey;
    }

    public IllegalAccountActionException(String errorMessage, String clientMessageKey, Throwable cause) {
        super(errorMessage, cause);
        this.clientMessageKey = clientMessageKey;
    }

    public String getClientMessageKey() {
        return clientMessageKey;
    }
}
