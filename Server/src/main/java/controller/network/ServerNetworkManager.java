...
public class ServerNetworkManager extends NetworkManager<Server> {
    ...
    public void start() {
        ...
        try {
            ...
            LOGGER.info(String.format("Hosted Server on tcp and udp port: %d, %d", this.tcp, this.udp));
        } catch (IOException ex) {
            this.active = false;

            LOGGER.warning(String.format("Failed to host Server on tcp and udp port: %d, %d", this.tcp, this.udp));
        }
    }
